...
class PadProWechatBotAdapter extends BotAdapter {
    ...
    async _newWechatyBot(loginInfo) {
        ...
        process.env.WECHATY_PUPPET_PADPLUS_TOKEN = config.token

        const puppet = new PuppetPadplus({
          token: config.token,
        })

        log.info('padproToken,', config.token)
        ...
        // return new Wechaty({
        //     puppet: 'wechaty-puppet-macpro',
        //     puppetOptions: {
        //         token: 'puppet_macpro_e34864e58e561ec2',
        //     },
        //     memory
        // });
    }
    ...
    async _responseLoginDone() {
        let userId = this.contactSelf.id
    ...
    _registerBotActions() {
        const _handleBotMessage = async (message) => {
            ...
            const handlersDict = {
                ...
                [MessageType.Url]: async (message) => {
                    ...
                    payload['content'] = {
                      msg: {
                        appmsg: JSON.parse(payload['content'])
                      }
                    }
        ...
        // emit when the bot needs to show you a QR Code for scanning
            ...
            .on('friendship', async (friendship) => {
                ...
                if(friendship.payload.type === this.wechatyBot.Friendship.Type.Receive){
    ...
    _registerHubActions() {
        ...
        this.registerHubAction("AcceptUser", async (payload) => {
            ...
            payload.type = this.wechatyBot.Friendship.Type.Receive
            const friendShip = this.wechatyBot.Friendship.load(payload.id);
            friendShip.payload = payload
            // await this.wechatyBot.puppet.cacheManager.setFriendshipRawPayload(friendShip.payload.id, friendShip.payload)
        ...
        this.registerHubAction("InviteRoomMember", async (actionBody) => {
            ...
            const contact = await this._findTargetById(userId)
            const room = await this._findTargetById(groupId)
            log.debug('InviteRoomMember', contact.id, room.id)
            await room.add(contact);
        });
